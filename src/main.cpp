#include <iostream>
#include "SFML/Graphics.hpp"
#include "Game.h"

#include "crtdbg.h"


int main()
{
  std::cout << "You should see a window that opens as well as this writing to console..."
            << std::endl;

  // create window and set up
  //sf::RenderWindow window(sf::VideoMode(1080, 720), "Platform game", sf::Style::Default);
  sf::RenderWindow window(sf::VideoMode(2560,1440), "Platform game", sf::Style::Fullscreen);
  //sf::RenderWindow window(sf::VideoMode(1920, 1080), "Platform game", sf::Style::Fullscreen);

  sf::View camera(sf::FloatRect(0.f, 0.f, 1920.f, 1080.f));
  //sf::View camera(sf::FloatRect(0.f, 0.f, (float)window.getSize().x, (float)window.getSize().y));
  //camera.setCenter(0, 0);
  //camera.setCenter((float)window.getSize().x/2, (float)window.getSize().y/2);
  camera.setCenter((float)camera.getSize().x/2, (float)camera.getSize().y/2);
  //camera.zoom(1080.f/camera.getSize().y);
  window.setView(camera);


  window.setFramerateLimit(60);
  //window.setFramerateLimit(120);

  //initialise an instance of the game class
  Game game(window);

  //run the init function of the game class and check it all initialises ok
  if (!game.init())
  {
    return 0;
  }

  // A Clock starts counting as soon as it's created
  sf::Clock clock;

  // Game loop: run the program as long as the window is open
  while (window.isOpen())
  {
    // check all the window's events that were triggered since the last iteration of the loop
    sf::Event event;

    //calculate delta time
    sf::Time time = clock.restart();
    float dt = time.asSeconds();

    //'process inputs' element of the game loop
    while (window.pollEvent(event))
    {
      // "close requested" event: we close the window
      if (event.type == sf::Event::Closed)
      {
        // _CrtDumpMemoryLeaks();
        window.close();
      }
      else if(event.type == sf::Event::KeyPressed)
      {
        game.keyPressed(event);
      }
      else if(event.type == sf::Event::KeyReleased)
      {
        game.keyReleased(event);
      }
      else if(event.type == sf::Event::MouseButtonPressed)
      {
        game.mouseClicked(event);
      }
    }


    //'update' element of the game loop
    game.update(dt);

    window.clear(sf::Color::Black);

    //'render' element of the game loop
    game.render();
    window.display();
  }
  return 0;
}
